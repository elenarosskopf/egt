...
## We check if we can assume to have a stable distribution: F Test and t-test (two sided):
...
## We check whether the distribution remain stable:
##First F test, then t test
var.test(exp06$propC_end, exp04$propC_end)
t.test(exp06$propC_end, exp04$propC_end)

## They are seem to fit, so we can assume that if cost and benefit structure is given, 
##the stable distribution of propC_end is evolving.
